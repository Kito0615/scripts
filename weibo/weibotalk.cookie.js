...
if (
  ...
  $.msg(
    "微博超话",
    "✅获取已关注超话列表成功",
    "✨接下来，请点进一个超话进行签到。如果没有需要签到的超话，请关注新的进行签到。"
  );
} else if (
  $request &&
  $request.method != "OPTIONS" &&
  ...
  $.msg(
    "微博超话",
    "🎉获取超话签到链接成功",
    `🚨请回到底部栏“关注”，上面切换到“关注”，从下往上滑，获取签到状态。如果$.check_first设为false则Cookie获取已完成，可以关闭获取Cookie的脚本或重写。`
  );
} else if (
  $request &&
  $request.method != "OPTIONS" &&
  ...
  $.msg(
    "微博超话",
    "🆗获取超话签到状态成功",
    `Cookie获取全部完成，可以关闭获取Cookie的脚本或重写。`
  );
}
